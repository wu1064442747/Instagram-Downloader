...
function addResultItem(url, quality, format) {
    ...
    // 创建下载链接（这里只是模拟，实际应用中应该连接到后端API）
    const downloadLink = `https://api.fastdl.space/download?url=${encodeURIComponent(url)}&quality=${quality}&format=${format}`;
    
    // 创建缩略图URL（这里只是模拟）
    const thumbnailUrl = `https://api.fastdl.space/thumbnail?url=${encodeURIComponent(url)}`;
